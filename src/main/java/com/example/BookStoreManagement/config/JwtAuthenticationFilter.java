...
//
//import com.ravish.ClinicServer.CurrentTenantId;
//import com.ravish.ClinicServer.config.JwtTokenProvider;
//import com.ravish.ClinicServer.service.UserService;
//
...
//	 @Autowired
//	 public JwtAuthenticationFilter(UserDetailsServiceImpl userService) {
...
//       CurrentTenantId.set(tenantId);
			if (StringUtils.hasText(jwt) && tokenProvider.validateToken(jwt)) {
				...
				String comanycode = (String) claims.get("username");
				CurrentTenantId.set(comanycode);
				String username = (String) claims.get("username");
				Integer userId = Integer.parseInt(claims.getSubject());
				 ...
				 * JWT. That would avoid the following database hit. It's completely up to you.
				 */
				UserDetails userDetails = userService.loadUserByUsername(comanycode);
				UsernamePasswordAuthenticationToken authentication = new UsernamePasswordAuthenticationToken(
						userDetails, null, userDetails.getAuthorities());
				authentication.setDetails(new WebAuthenticationDetailsSource().buildDetails(request));
